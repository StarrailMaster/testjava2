...
@Service
public class CustomerRecommendService {
    ...
    public String generateCustomerRecommend(String SearchText) {
        if (apiKey == null) {
        ...
        String prompt = String.format(
                "You are an AI customer support assistant for an intelligent e-commerce platform.\n" +
                        "Based on the user's inquiry below, provide relevant product recommendations:\n\n" +
                        "User Inquiry: \"%s\"\n\n" +
                        "Recommendations:\n" +
                        "1. [Recommendation 1]\n" +
                        "2. [Recommendation 2]\n" +
                        "3. [Recommendation 3]\n" +
                        "Please provide a brief description of each recommendation and how it meets the user's needs.",
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                ...
                return "Failed to generate customer recommend.";
            }
        } catch (Exception e) {
